...
class ProfessionalDrumEngine {
  ...
  // ─── Métodos At: agendam no tempo absoluto do AudioContext ─────────────────
  // Cada método salva currentTime, substitui temporariamente e restaura

  _withTime(time, fn) {
    if (time === undefined || time === null) { fn(); return; }
    // Monkey-patch temporariamente currentTime para que os métodos usem o tempo agendado
    const origDescriptor = Object.getOwnPropertyDescriptor(Object.getPrototypeOf(this.audioContext), 'currentTime');
    // Não é possível sobrescrever currentTime diretamente (readonly).
    // Em vez disso, passamos o offset para os métodos via _scheduledTime
  ...
  _now() {
    return this._scheduledTime !== undefined && this._scheduledTime !== null
      ? this._scheduledTime
      : this.audioContext.currentTime;
  }

  playKickAt(style, velocity, time) {
    this._scheduledTime = time;
    this._playKickScheduled(style, velocity);
    this._scheduledTime = null;
  }

  playSnareAt(style, velocity, time) {
    this._scheduledTime = time;
    this._playSnareScheduled(style, velocity);
    this._scheduledTime = null;
  }

  playHihatClosedAt(style, velocity, time) {
    this._scheduledTime = time;
    this._playHihatClosedScheduled(style, velocity);
    this._scheduledTime = null;
  }

  playHihatOpenAt(style, velocity, time) {
    this._scheduledTime = time;
    this._playHihatOpenScheduled(style, velocity);
    this._scheduledTime = null;
  }

  playCrashAt(velocity, time) {
    this._scheduledTime = time;
    this._playCrashScheduled(velocity);
    this._scheduledTime = null;
  }

  playRideAt(velocity, time) {
    this._scheduledTime = time;
    this._playRideScheduled(velocity);
    this._scheduledTime = null;
  }

  playTomAt(pitch, velocity, time) {
    this._scheduledTime = time;
    this._playTomScheduled(pitch, velocity);
    this._scheduledTime = null;
  }

  // Versões internas que usam _now() em vez de audioContext.currentTime
  _playKickScheduled(style = 'acoustic', velocity = 1.0) {
    const now = this._now();
    const osc = this.audioContext.createOscillator();
    const oscGain = this.audioContext.createGain();
    const clickOsc = this.audioContext.createOscillator();
    ...
    if (style === 'electronic') {
      ...
      clickOsc.type = 'square'; clickOsc.frequency.setValueAtTime(2000, now); clickOsc.frequency.exponentialRampToValueAtTime(100, now + 0.005);
      noiseFilter.frequency.setValueAtTime(300, now);
      oscGain.gain.setValueAtTime(velocity * 1.2, now); oscGain.gain.exponentialRampToValueAtTime(0.01, now + 0.3);
      clickGain.gain.setValueAtTime(velocity * 0.7, now); clickGain.gain.exponentialRampToValueAtTime(0.01, now + 0.005);
      noiseGain.gain.setValueAtTime(velocity * 0.15, now); noiseGain.gain.exponentialRampToValueAtTime(0.01, now + 0.05);
      osc.connect(oscGain); oscGain.connect(this.masterGain);
      clickOsc.connect(clickGain); clickGain.connect(this.masterGain);
      noise.connect(noiseFilter); noiseFilter.connect(noiseGain); noiseGain.connect(this.masterGain);
      osc.start(now); clickOsc.start(now); noise.start(now);
      osc.stop(now + 0.3); clickOsc.stop(now + 0.01); noise.stop(now + 0.1);
    } else {
      const dur = style === 'rock' ? 0.4 : 0.5;
      const startFreq = style === 'rock' ? 180 : 150;
      osc.frequency.setValueAtTime(startFreq, now); osc.frequency.exponentialRampToValueAtTime(40, now + dur);
      clickOsc.frequency.setValueAtTime(1000, now); clickOsc.frequency.exponentialRampToValueAtTime(100, now + 0.01);
      noiseFilter.frequency.setValueAtTime(200, now);
      oscGain.gain.setValueAtTime(velocity, now); oscGain.gain.exponentialRampToValueAtTime(0.01, now + dur);
      clickGain.gain.setValueAtTime(velocity * 0.5, now); clickGain.gain.exponentialRampToValueAtTime(0.01, now + 0.01);
      noiseGain.gain.setValueAtTime(velocity * 0.1, now); noiseGain.gain.exponentialRampToValueAtTime(0.01, now + 0.1);
      osc.connect(oscGain); oscGain.connect(this.masterGain);
      clickOsc.connect(clickGain); clickGain.connect(this.masterGain);
      noise.connect(noiseFilter); noiseFilter.connect(noiseGain); noiseGain.connect(this.masterGain);
      osc.start(now); clickOsc.start(now); noise.start(now);
      osc.stop(now + dur); clickOsc.stop(now + 0.02); noise.stop(now + 0.2);
    }
  }

  _playSnareScheduled(style = 'acoustic', velocity = 1.0) {
    const now = this._now();
    const osc1 = this.audioContext.createOscillator();
    const osc2 = this.audioContext.createOscillator();
    const oscGain = this.audioContext.createGain();
    ...
    const f1 = style === 'rock' ? 190 : style === 'electronic' ? 200 : 180;
    const f2 = style === 'rock' ? 350 : style === 'electronic' ? 400 : 330;
    osc1.frequency.setValueAtTime(f1, now); osc1.frequency.exponentialRampToValueAtTime(90, now + 0.15);
    osc2.frequency.setValueAtTime(f2, now); osc2.frequency.exponentialRampToValueAtTime(140, now + 0.15);
    noiseFilter.frequency.setValueAtTime(style === 'electronic' ? 1500 : 1000, now);
    oscGain.gain.setValueAtTime(velocity * 0.4, now); oscGain.gain.exponentialRampToValueAtTime(0.01, now + 0.2);
    noiseGain.gain.setValueAtTime(velocity * 0.6, now); noiseGain.gain.exponentialRampToValueAtTime(0.01, now + 0.15);
    osc1.connect(oscGain); osc2.connect(oscGain); oscGain.connect(this.masterGain);
    noise.connect(noiseFilter); noiseFilter.connect(noiseGain); noiseGain.connect(this.masterGain);
    osc1.start(now); osc2.start(now); noise.start(now);
    osc1.stop(now + 0.2); osc2.stop(now + 0.2); noise.stop(now + 0.2);
  }

  _playHihatClosedScheduled(style = 'acoustic', velocity = 1.0) {
    const now = this._now();
    const noise = this.createNoise();
    const noiseGain = this.audioContext.createGain();
    const filter = this.audioContext.createBiquadFilter();
    filter.type = style === 'acoustic' ? 'bandpass' : 'highpass';
    filter.frequency.setValueAtTime(style === 'acoustic' ? 8000 : 7000, now);
    filter.Q.setValueAtTime(style === 'acoustic' ? 2 : 1, now);
    noiseGain.gain.setValueAtTime(velocity * 0.5, now);
    noiseGain.gain.exponentialRampToValueAtTime(0.01, now + 0.05);
  ...
  _playHihatOpenScheduled(style = 'acoustic', velocity = 1.0) {
    const now = this._now();
    const noise = this.createNoise();
    const noiseGain = this.audioContext.createGain();
    const filter = this.audioContext.createBiquadFilter();
  ...
  _playCrashScheduled(velocity = 1.0) {
    const now = this._now();
    const noise = this.createNoise();
    const noiseGain = this.audioContext.createGain();
    const filter = this.audioContext.createBiquadFilter();
  ...
  _playRideScheduled(velocity = 1.0) {
    const now = this._now();
    const noise = this.createNoise();
    const noiseGain = this.audioContext.createGain();
    const filter = this.audioContext.createBiquadFilter();
  ...
  _playTomScheduled(pitch = 'mid', velocity = 1.0) {
    const now = this._now();
    const freqMap = { high: 200, mid: 130, low: 80 };
    const freq = freqMap[pitch] || 130;
    const osc = this.audioContext.createOscillator();
