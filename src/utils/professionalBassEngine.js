...
class ProfessionalBassEngine {
  ...
  /**
   * Tocar nota em um tempo absoluto agendado (Web Audio API lookahead)
   * @param {string} note - Nome da nota (ex: 'E', 'A', 'C#')
   * @param {number} octave - Oitava (1 = baixo padrão)
   * @param {string} style - 'fingerstyle' | 'slap' | 'pick'
   * @param {number} duration - Duração em segundos
   * @param {number} velocity - Volume 0-1
   * @param {number} time - AudioContext.currentTime absoluto
   */
  playNoteAt(note, octave, style = 'fingerstyle', duration = 1.0, velocity = 1.0, time = null) {
    const scheduledTime = time !== null ? time : this.audioContext.currentTime;
    this._playNoteAtTime(note, octave, style, duration, velocity, scheduledTime);
  }

  _playNoteAtTime(note, octave, style, duration, velocity, startTime) {
    const frequency = this.getNoteFrequency(note, octave);
    const attackTime  = style === 'slap' ? 0.001 : style === 'pick' ? 0.005 : 0.01;
    const decayTime   = style === 'slap' ? 0.05  : style === 'pick' ? 0.08  : 0.1;
    const releaseTime = style === 'slap' ? 0.2   : style === 'pick' ? 0.25  : 0.3;
    const sustainLevel = velocity * (style === 'slap' ? 0.5 : 0.6);
    const filterFreq   = style === 'slap' ? 2000 : style === 'pick' ? 1200 : 800;
    const filterQ      = style === 'slap' ? 2    : style === 'pick' ? 1.5  : 1;
    ...
    harmonic2.type = style === 'slap' ? 'sawtooth' : 'triangle';
    ...
    harm2Gain.gain.linearRampToValueAtTime(velocity * 0.3, startTime + attackTime);
    harm2Gain.gain.linearRampToValueAtTime(velocity * 0.15, startTime + attackTime + decayTime);
    harm2Gain.gain.setValueAtTime(velocity * 0.15, startTime + duration);
    harm2Gain.gain.exponentialRampToValueAtTime(0.001, startTime + duration + releaseTime);
    ...
    this.activeNotes.push({ oscillators: [fundamental, harmonic2], stopTime });
    this.cleanupActiveNotes();
  }
